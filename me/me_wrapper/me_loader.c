...
#define RETURN_QT (256)

enum MARKET_TYPE{ KDB_QUOTE = 1, NUMPY_QUOTE = 2 };
static char s_perf_log[MAX_LOG_LENGTH];
...
me_create(me_config_t* config)
{
    ...
    add_time_record("trader_init", end - start);
#endif
    if(!me){
    ...
    // store perf log file
    strcpy(s_perf_log, config->perf_log);
    return handle;
}
...
py_me_feed_quote(PyObject *self, PyObject *args)
{
    ...
    PyArrayObject *arg2;
    if (!PyArg_ParseTuple(args, "KOII:feed_quote", &me_ptr, &arg2, &qindex, &is_bar)){
        PyErr_Format(PyExc_TypeError,"Match engine feed quote failed: parameter not match.");
        return NULL;
    }
...
#define MI_DCE_ORDER_STATISTIC  3
    ...
    add_time_record("convert resp", end - start);
#endif
    return root;
}
...
py_me_feed_order(PyObject *self, PyObject *args)
{
    ...
    for(i=0; i<order_len; ++i){
        ...
        add_time_record("convert_ord", end - start);
#endif
        ...
        add_time_record("convert_order", end - start);
#endif
    ...
    add_time_record("convert_rsp", end - start);
#endif
    return root;
}
...
py_me_destroy(PyObject *self, PyObject *args)
{
    ...
    add_time_record("me_destroy", end - start);
#endif
    Py_XDECREF(h->conf_arg);
    free(h);
#ifdef PROFILE
    stat_time_record(s_perf_log);
#endif
    Py_RETURN_NONE;
}
