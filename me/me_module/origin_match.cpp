...
#define  GET_ST_ID(serial_no) (serial_no % 1000)
#define  MAX_STRAT_SIZE       (1024 * 256) 
...
void  
OriginMatch::place_order(place_order_struct* pl_ord, order_return_struct *ord_rtn)
{
	if (pl_ord->volume <= 0) {
...
void  
OriginMatch::cancel_order(cancel_order_struct* cl_ord, order_return_struct *ord_rtn)
{
	order_node *on = NULL;
	strat_node *sn = NULL;
	tick_node  *tn = NULL;
	...
	auto need = m_place_hash->find(cl_ord->org_serial_no);
	...
	if (on->status == STAT_FULL
	|| on->status == STAT_CANCEL){
		ord_rtn->entrust_status = STAT_CANCEL_REJECT;
		goto canl_end;
	}
	...
	on->status = STAT_CANCEL;
	ord_rtn->entrust_status = on->status;
	m_place_hash->erase(need);

canl_end:
	ord_rtn->serial_no = on->pl_ord.serial_no;
	memcpy(ord_rtn->contr, on->pl_ord.contr, MAX_NAME_SIZE);
	ord_rtn->direction = on->pl_ord.direction;
	ord_rtn->open_close = on->pl_ord.open_close;
	ord_rtn->entrust_no = on->entrust_no;
	ord_rtn->xchg_code = on->pl_ord.xchg_code;
	ord_rtn->volume = on->pl_ord.volume;
	ord_rtn->volume_remain = on->volume;
}

void
OriginMatch::feed_quote(int type, void* quote, bool is_first)
{
	m_mi_type = type;
	m_is_filter = Filter;
	if (NULL == quote_process[type]){
		return;
	}
...
void  OriginMatch::feed_internal_book(void* quote, bool is_first)
{
	m_tick_counter++;
	m_is_filter = Filter;
	m_last_quote = quote;
...
void
OriginMatch::execute_match(trade_return_struct** trd_rtn_ar, int* rtn_cnt)
{
...
price_node *
OriginMatch::ogn_find_price_node(double price, char direct)
{
	struct list_head *head, *pos;
	struct price_node *pn;
	better_price_func cmp_func;
	head = get_price_handle(direct);
	cmp_func = compare_price[direct - DIR_BUY];
...
price_node*
OriginMatch::ogn_add_price_node(double price, char direct)
{
...
OriginMatch::ogn_add_strat_node(tick_node *tn, int st_id)
{
	...
	if (NULL == sn) {
		printf("Error: Can't get available strat_node\n");
		return NULL;
	}
	...
	list_for_each(pos, head) {
		...
		if (st_id < tmp->st_id) {
			...
			printf("ERROR: add duplicated strat_node:%d\n", st_id);
		}
	}
...
order_node *
OriginMatch::ogn_add_order_node(strat_node *sn, place_order_struct *ord)
{
...
bool  
OriginMatch::ogn_update_order_book(place_order_struct *pl_ord)
{
	...
	strat_node* sn = ogn_find_strat_node(tn, GET_ST_ID(pl_ord->serial_no));
	if (NULL == sn) {
		sn = ogn_add_strat_node(tn, GET_ST_ID(pl_ord->serial_no));
	}
	if (sn == NULL){
		return false;
	}
	...
	on->entrust_no = m_entrust_num;
	sn = on->the_st;
	sn->vol += pl_ord->volume;
	tn = sn->parent;
	tn->volume += pl_ord->volume;
	pn = tn->parent;
	pn->volume += pl_ord->volume;
	m_place_hash->insert(std::make_pair(pl_ord->serial_no, on));
	return true;
}

void 
OriginMatch::ogn_match_on_dir(char direct)
{
	struct list_head *pos, *p, *head;
...
void  
OriginMatch::update_mkt_tick(price_node *pn, char direct)
{
	int new_vol_mkt;
	int travel_vol = 0;
	struct tick_node *tn;
...
void  
OriginMatch::update_tradeable_vol(pv_pair *pv_ar, int cnt, double my_price, char dir, int vol)
{
	for (int i = 0; i < cnt; i++) {
		pv_pair *pv = pv_ar + i;
		if (if_equal_trade(pv->price, my_price, dir)) {
...
void 
OriginMatch::update_opposite_vol(char dir, int vol)
{
	struct pv_pair *pv_ar = m_quote.opposite.bs_pv[dir - DIR_BUY];
...
int
OriginMatch::calc_opp_trade_vol(double my_price, char direct)
{
	int opp_vol = 0;
	if (EXCH_CZCE == m_cfg.pi.xchg_code) {
		...
		if (strcmp(m_quote.curr_quote.time, m_quote.last_quote.time) == 0) {
			return 0;
		}
	}
...
int   
OriginMatch::if_equal_trade(double mkt_price, double my_price, char dir)
{
	return (compare(mkt_price, my_price) == 0
		|| is_can_trade(mkt_price, my_price, dir));
}

int   
OriginMatch::calc_tradeable_vol(pv_pair *pv_ar, int cnt, double price, char direct)
{
	int trade_vol = 0;
	for (int i = 0; i < cnt; i++) {
		pv_pair *pv = pv_ar + i;
...
int  
OriginMatch::get_vol_on_price(double price, int volume, char direct)
{
	struct pv_pair *pv_ar = m_quote.curr_quote.bs_pv[direct - DIR_BUY];
...
int 
OriginMatch::get_volume_prior(place_order_struct *pl_ord)
{
	struct mat_quote *cur_q = &(m_quote.curr_quote);
	struct pv_pair *pv_ar = cur_q->bs_pv[pl_ord->direction - DIR_BUY];
	for (int i = 0; i < QUOTE_LVL_CNT; i++) {
